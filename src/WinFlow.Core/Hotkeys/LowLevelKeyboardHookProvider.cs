...
public sealed class LowLevelKeyboardHookProvider : IHotkeyProvider
{
    ...
    private const uint WmForceRelease = 0x0400; // WM_USER
    ...
    // the key-up never arrives and _keyIsDown would stay true forever. Session
    // switches are handled via <see cref="NotifyDesktopSwitch"/> instead of a
    // periodic timer — Ctrl keys do not typematic-repeat, so tick-based resync
    // falsely ended recording while Right Ctrl was still held.
    ...
    /// <summary>
    /// Forces a synthetic <see cref="HotkeyEvent.Released"/> when the watched
    /// key-up was delivered to a desktop this hook cannot observe (UAC, lock
    /// screen). Safe to call from any thread.
    /// </summary>
    public void NotifyDesktopSwitch()
    {
        if (_hookThreadId != 0)
        {
            PostThreadMessage(_hookThreadId, WmForceRelease, 0, 0);
        }
    }
    ...
    private void HookThreadMain(TaskCompletionSource<Exception?> ready)
    {
        ...
        {
            if (msg.Message == WmForceRelease)
            {
                ForceReleaseOnHookThread();
            }
        }
    ...
    /// desktop this hook cannot observe. Runs on the hook thread.
    /// </summary>
    private void ForceReleaseOnHookThread()
    {
        if (!_keyIsDown)
        {
            return;
